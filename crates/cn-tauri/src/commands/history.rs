...
pub async fn get_history(state: State<'_, AppState>) -> Result<Vec<HistoryEntry>, String> {
    ...
    tokio::task::spawn_blocking(move || store.get_history().map_err(|e| e.to_string()))
        .await
...
pub async fn clear_history(state: State<'_, AppState>) -> Result<(), String> {
    let store = state.history_store.clone();
    tokio::task::spawn_blocking(move || store.clear().map_err(|e| e.to_string()))
        .await
        .map_err(|e| e.to_string())?
}
...
pub async fn undo_operation(
    ...
    let res =
        tokio::task::spawn_blocking(move || undo_execution(&result).map_err(|e| e.to_string()))
            .await
            .map_err(|e| e.to_string())??;
