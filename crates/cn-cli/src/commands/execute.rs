use cn_core::{
    ...
    models::{
        ...
        operation::{OrganizeMode, DateGranularity, SizeThresholds},
...
pub async fn run(
    ...
    let mode = match mode_str.to_lowercase().as_str() {
        ...
        "bydate" => OrganizeMode::ByDate { granularity: DateGranularity::Month },
        "bysize" => OrganizeMode::BySize { thresholds: SizeThresholds::default() },
        _ => anyhow::bail!("Invalid organize mode. Use 'smart', 'extension', 'category', 'bydate', 'bysize'."),
    };
    ...
    if !is_json {
        println!("\nPlan ready: {} files ({} bytes) will be moved.", plan.total_files(), plan.estimated_bytes);
    }
    ...
    tokio::spawn(async move {
        ...
        while let Some(progress) = rx.recv().await {
            if let Some(pb) = &pb_clone {
                if let cn_core::executor::ExecuteProgressEvent::Finished { bytes_processed, .. } = progress {
    ...
    if is_json {
        ...
        let errors: Vec<_> = result.file_results.iter().filter(|r| r.outcome == FileOutcome::Failed).collect();
        if !errors.is_empty() {
