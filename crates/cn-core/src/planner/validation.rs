...
pub fn validate(
    ...
    let dest_canon = destination_dir.canonicalize().unwrap_or_else(|_| destination_dir.to_path_buf());
    ...
    for op in operations {
        ...
        let op_canon = check_path.canonicalize().unwrap_or_else(|_| check_path.to_path_buf());
    ...
    let active_ops = operations.iter().filter(|op| op.kind != OperationKind::Skip).count();
    if active_ops == 0 {
